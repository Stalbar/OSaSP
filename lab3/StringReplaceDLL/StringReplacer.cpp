...
void Replace(const char* data, const char* replacement)
{
	...
	while (p < si.lpMaximumApplicationAddress)
	{
		...
		if (info.State == MEM_COMMIT && info.AllocationProtect == PAGE_READWRITE)
		{
			...
			readedInfo.resize(info.RegionSize);
			SIZE_T bytesRead;
			if (ReadProcessMemory(process, p, &readedInfo[0], info.RegionSize, &bytesRead) != 0)
			{
				for (size_t i = 0; i < (bytesRead - len); ++i)
				{
					if (memcmp(data, &readedInfo[i], len) == 0)
					{
						char* ref = (char*)p + i;
						int j;
